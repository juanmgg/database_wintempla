#include "stdafx.h"  //_____________________________________________ admDlg.cpp
#include "admDlg.h"

void admDlg::Window_Open(Win::Event& e)
{
	...
	try
	{
		...
		if (administrador_id < 0) return;
		Sys::Format(cmd, L"SELECT nombre, cedula, nss, rfc, domicilio, correo, telefono, usuario, password_ FROM administrador WHERE administrador_id=%d", administrador_id);
...
void admDlg::btOK_Click(Win::Event& e)
{
	...
	Sql::StringBuilder sb(L"administrador", L"administrador_id", administrador_id);
...
void admDlg::btCancel_Click(Win::Event& e)
{
